...
bool RoleService::isRoleIDExists(const oatpp::String& id)
{
    ...
    try {
        // 使用直接的数据库查询检查角色是否存在，只查询count而不获取完整对象
        std::ostringstream sql;
        sql << "SELECT COUNT(*) FROM role WHERE id = ?";

        SqlParams params;
        // 安全转换oatpp::String为std::string
        std::string roleId = *id;
        SQLPARAMS_PUSH(params, "s", std::string, roleId);

        SqlSession sqlSession;
        uint64_t count = sqlSession.executeQueryNumerical(sql.str(), params);
