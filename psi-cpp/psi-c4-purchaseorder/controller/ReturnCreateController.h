#pragma once
#ifndef _RETURN_CREATE_CT_
#define _RETURN_CREATE_CT_
...
//获取采购单报表控制器
class ReturnCreateController :public oatpp::web::server::api::ApiController // 继承控制器
{
	// 定义控制器访问入口
	API_ACCESS_DECLARE(ReturnCreateController);
	// 定义接口
public:
	//定义接口描述
	ENDPOINT_INFO(queryReturnCreate)
	{
		//定义接口标题
		API_DEF_ADD_TITLE(ZH_WORDS_GETTER("potable.return_create.summary"));
	...
	//定义查询接口处理
	ENDPOINT(API_M_GET, "/purchaseorder/return_create", queryReturnCreate, QUERIES(QueryParams, queryParams), API_HANDLER_AUTH_PARAME)
	{
		// 解析查询参数为Query领域模型
