#pragma once
#ifndef _DINGDANDO_H_
#define _DINGDANDO_H_

#include "./DoInclude.h"
#include "./FileDO.h"

// 采购订单主表DO - 对应bor表
class BorDO : public BaseDO {
	// 订单ID
	...
	// 关联一个文件实体，存储单据附件信息
	CC_SYNTHESIZE(PtrFileDO, fileUrl, FileUrl);
public:
	BorDO() : BaseDO("bor") {
		...
		MYSQL_ADD_FIELD("arrival", "s", arrival);
...
//// 兼容性定义 - 保持向后兼容
//typedef BorDO DingdanDO;

typedef std::shared_ptr<BorDO> PtrBorDO;
...
// 兼容性定义 - 保持向后兼容
typedef BorInfoDO DingdanDetailDO;
